...
def menu_kb(prem, admin):
    ...
    if not prem:
        kb.append([InlineKeyboardButton(text="💰 Premium", callback_data="premium")])
    else:
...
async def query_set(m: Message, state: FSMContext):
    await state.update_data(q=m.text)
    await m.answer("Seç:", reply_markup=budget_kb())
...
async def choose_type(cb: CallbackQuery, state: FSMContext):
    ...
    if not q:
        await cb.message.edit_text("Önce ürün yaz.")
        await state.set_state(States.query)
        return

    if cb.data == "budget":
        await cb.message.edit_text("Min fiyat:")
        ...
        if not items:
            await cb.message.edit_text("Ürün yok.", reply_markup=back_kb())
            return
...
async def min_price(m: Message, state: FSMContext):
    try:
        val = float(m.text)
    except:
    ...
    await m.answer("Max fiyat:")
    await state.set_state(States.max_price)

# ================= MAX =================
@dp.message(States.max_price)
async def max_price(m: Message, state: FSMContext):
    try:
        val = float(m.text)
    except:
        await m.answer("Sayı gir")
